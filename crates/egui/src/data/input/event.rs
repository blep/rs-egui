...
/// Data for a stylus/pen event.
///
/// Emitted as `Event::Stylus(...)` when the platform can distinguish pen
/// input from finger touch. Fields that overlap with `Event::Touch` use
/// identical names and types (`pos`, `force`).
...
pub struct StylusData {
    ...
    /// Normalised pressure [0, 1]. Same as `Event::Touch.force`.
    /// `None` = hovering (pen is detected but not in contact).
    ...
    /// screen plane. Range [-π, π].
    pub orientation: Option<f32>,

    /// Hover distance (≥ 0). Unit is platform-dependent.
    pub hover_distance: Option<f32>,

    /// The type of tool generating this input.
...
impl StylusData {
    /// `true` if the pen is detected but hovering above the surface
    /// (not in contact). During hover, `force` is `None`.
...
pub enum Event {
    ...
    /// pen or stylus hover/move/up/down with full axis data.
    ///
    /// This event is emitted *instead of* [`Event::Touch`] for pen
    /// and stylus input, and only on platforms that can distinguish
    /// pen from finger (Android, Windows, Linux via libinput).
    Stylus(StylusData),
